...
class DeviceConnector:
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.room = self.config['location'] 
        ...
        # Initialize HVAC status and mode for the single room
        ...
        self.real_temperature = None  # Actual temperature for the room
        self.simulated_temperature = None  # Simulated temperature for the room

        # Initialize sensors
        self.dht11_sensor = SimulatedDHT11Sensor(self.room)
    ...
    def publish_environment_data(self, input_data):
        ...
        try:
            ...
            if temperature is not None:
                self.real_temperature[self.room] = temperature  # Update the actual temperature from the sensor

                # Modify temperature based on HVAC status and residual effect
                modified_temperature = self.update_simulated_temperature(self.room)
            ...
            topic = self.config["published_topics"]["environment"].replace('<roomID>', self.room)
    ...
    def GET(self, *uri, **params):
        """
        Handle GET requests to provide data to the Telegram bot.
        """
        if len(uri) == 0:
            return {"status": "error", "message": "No endpoint specified."}

        if uri[0] == "environment":
            # Retrieve environmental data for all rooms or a specific room
            if "room" in params:
                room = params["room"]
                temperature = self.simulated_temperature.get(room, None)
                humidity = self.real_temperature.get(room, None)

                if temperature is None or humidity is None:
                    return {"status": "error", "message": f"No data available for room: {room}"}

                # Provide data in a format compatible with the bot
                return {
                    "status": "success",
                    "senml_record": {
                        "e": [
